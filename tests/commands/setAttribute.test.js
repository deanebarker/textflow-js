...
test("Sets an attribute on an element", async () => {
  const command = { name: "set-attribute", selector: "img", attribute: "alt", value: "A photo" };
...
test("Replaces an existing attribute value", async () => {
  const command = { name: "set-attribute", selector: "a", attribute: "href", value: "https://new.com" };
...
test("Sets a data attribute", async () => {
  const command = { name: "set-attribute", selector: "div", attribute: "data-id", value: "42" };
...
test("Modifies all matching elements by default", async () => {
  const command = { name: "set-attribute", selector: "p", attribute: "class", value: "highlight" };
...
test("Modifies all matching elements when limit is 0", async () => {
  const command = { name: "set-attribute", selector: "p", attribute: "class", value: "highlight", limit: 0 };
...
test("Sets attribute on first N elements when limit is provided", async () => {
  const command = { name: "set-attribute", selector: "p", attribute: "class", value: "highlight", limit: 2 };
  const result = await execute([command], "<p>a</p><p>b</p><p>c</p>");
...
test("Sets attribute on only the first element when limit is 1", async () => {
  const command = { name: "set-attribute", selector: "p", attribute: "class", value: "highlight", limit: 1 };
  const result = await execute([command], "<p>first</p><p>second</p>");
...
test("Does not change text when selector matches nothing", async () => {
  const command = { name: "set-attribute", selector: "span", attribute: "class", value: "x" };
