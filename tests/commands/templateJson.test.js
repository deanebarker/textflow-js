...
test("Renders a simple template with JSON data", async () => {
  const command = { name: "template-json", template: "<p>{{ data.name }}</p>" };
...
test("Renders a template with multiple fields", async () => {
  const command = { name: "template-json", template: "<p>{{ data.first }} {{ data.last }}</p>" };
...
test("Renders a template iterating over an array", async () => {
  const command = { name: "template-json", template: "{% for item in data %}<li>{{ item }}</li>{% endfor %}" };
